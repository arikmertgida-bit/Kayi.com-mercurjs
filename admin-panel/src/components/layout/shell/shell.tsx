...
const DesktopSidebarContainer = ({ children }: PropsWithChildren) => {
  ...
  return (
    <div
      className={clx("hidden h-screen w-[220px] border-e border-ui-border-base bg-ui-bg-subtle", {
        "lg:flex": desktop,
      })}
    >
      {children}
  ...
  return (
    <RadixDialog.Root open={mobile} onOpenChange={() => toggle("mobile")}>
      <RadixDialog.Portal>
        ...
        <RadixDialog.Content
          className={clx(
            "fixed inset-y-2 start-2 flex w-full max-w-[304px] flex-col overflow-hidden rounded-lg border border-ui-border-base bg-ui-bg-base shadow-elevation-modal",
