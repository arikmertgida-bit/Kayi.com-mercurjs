...
const sortItems = (
  ...
  return items.sort((a, b) => {
    ...
    if (field === "product_title") {
      ...
      aValue = a.quantity - ((a.detail?.return_requested_quantity ?? 0) + (a.detail?.return_received_quantity ?? 0) + (a.detail?.return_dismissed_quantity ?? 0))
      bValue = b.quantity - ((b.detail?.return_requested_quantity ?? 0) + (b.detail?.return_received_quantity ?? 0) + (b.detail?.return_dismissed_quantity ?? 0))
    } else if (field === "refundable_amount") {
      aValue = Number(a.refundable_total) || 0
      bValue = Number(b.refundable_total) || 0
    }
...
const filterByNumber = (
  ...
  return items.filter((i) => {
    const returnableQuantity = i.quantity - ((i.detail?.return_requested_quantity ?? 0) + (i.detail?.return_received_quantity ?? 0) + (i.detail?.return_dismissed_quantity ?? 0))
    const refundableAmount = getStylizedAmount(Number(i.refundable_total) || 0, currency_code)
