...
const translateBackendError = (
  backendErrorKey: BackendErrorKey,
  params: Record<string, string>,
  fallback: string
): string => {
  const translated = i18n.t(`backendErrors.${backendErrorKey}`, {
    ...params,
...
export const mapBackendErrorMessage = (message: string): string => {
  const result = findBackendError(message)
  if (!result) {
    return message
  }

  return translateBackendError(result.key, result.params, message)
}
...
export const mapUnknownBackendError = (
  ...
  return mapBackendErrorMessage(extractedMessage)
}
