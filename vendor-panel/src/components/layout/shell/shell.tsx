...
const DesktopSidebarContainer = ({ children }: PropsWithChildren) => {
  ...
  return (
    <div
      className={clx("hidden h-full w-[220px] border-r border-ui-border-base bg-ui-bg-subtle", {
        "lg:flex": desktop,
      })}
    >
      {children}
  ...
  return (
    <RadixDialog.Root open={mobile} onOpenChange={() => toggle("mobile")}>
      <RadixDialog.Portal>
        ...
        <RadixDialog.Content
          className={clx(
            "fixed inset-y-2 left-2 flex w-full max-w-[304px] flex-col overflow-hidden rounded-lg border border-ui-border-base bg-ui-bg-base shadow-elevation-modal",
