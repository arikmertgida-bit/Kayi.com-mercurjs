...
export async function generateMetadata(): Promise<Metadata> {
  return {
    title: "Şimdi İndirimde | Kayı.com",
    description: "Aktif kampanyalar ve indirimli ürünler — fırsatları kaçırmadan sepete ekle.",
...
export default async function ActiveCampaignsPage() {
  ...
  const [{ campaigns }, megaMenuCategories] = await Promise.all([
    getCampaigns("active", 24, 0),
    listMegaMenuCategories().catch(() => []),
  ])

  const breadcrumbItems = [
    { path: "/", label: "Ana Sayfa" },
    { path: "/campaigns/active", label: t("onSale") },
  ]
        ...
        <div className="w-full">
          {campaigns.length === 0 ? (
            <div className="py-16 text-center text-gray-400">
              ...
              <p className="font-semibold text-lg">Şu an aktif bir kampanya bulunmuyor.</p>
              <p className="text-sm mt-1">Yakında başlayacak kampanyalar için sık sık kontrol edin!</p>
            </div>
          ) : (
